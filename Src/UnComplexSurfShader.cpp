...
void FD3DSurfShader::WriteConstantBuffer(void* InMem)
{
	...
	for (INT i = 0; i < 5; i++)
	{
		...
		if (ParentDevice->BoundTextures[i].TexInfoHash.Value != 0)
		{
	...
	if (ParentDevice->BoundTextures[1].TexInfoHash.Value != 0)
	{
	...
	if (ParentDevice->BoundTextures[3].TexInfoHash.Value != 0)
	{
